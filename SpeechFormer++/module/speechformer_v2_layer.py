...
from adapter.adapters import MetaLayersAdapterController, AdapterConfig
...
class SpeechFormer_v2_Encoder(nn.Module):
    ...
    def __init__(self, embed_dim=1024, ffn_embed_dim=2304, local_size=0, num_heads=8, 
            dropout=0.1, attention_dropout=0.1, activation='relu', overlap=True, num_wtok=0, train_adapters=False, adapert_config=None):
        super().__init__()
        ...
        self.adapter_after_attention = None
        # DONE added adapter here
        if self.train_adapters:
            self.adapter_after_attention = MetaLayersAdapterController(adapert_config)
        ...
        self.adapter_after_feed_forward = None
        # DONE added adapter here
        if self.train_adapters:
            self.adapter_after_feed_forward = MetaLayersAdapterController(adapert_config)
        self.final_layer_norm = nn.LayerNorm(embed_dim)

    def forward(self, x, x_position=None, kmeans_mask=None, task=None, block_adapters=None,):
        """
        ...
        # DONE added adapter call here
        if self.train_adapters:
            x = self.adapter_after_attention(x, block_adapters.self_attention)
        ...
        # DONE added adapter call here
        if self.train_adapters:
            x = self.adapter_after_feed_forward(x, block_adapters.feed_forward)
