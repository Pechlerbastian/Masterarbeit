...
class MetaLayersAdapterController(nn.Module):
    ...
    def call_adapter(self, inputs, adapter_weights):
        # print("Shapes before F.linear in call_adapter:")
        # # inputs = inputs.squeeze(0)
        # # down = adapter_weights.down.weight.t()
        # print("inputs:", inputs.size())
        # print("down.weight:",  adapter_weights.down.weight.size())
        # print("down.bias:", adapter_weights.down.bias.size())

        down = F.linear(
            inputs, weight=adapter_weights.down.weight, bias=adapter_weights.down.bias
        )
        
        # print("Shapes after the first F.linear:")
        # print("down:", down.size())
        
        middle = get_activation(self.activation_type)(down)
        
        # print("Shapes before the second F.linear:")
        # print("middle:", middle.size())
        # print("up.weight:", adapter_weights.up.weight.size())
        # print("up.bias:", adapter_weights.up.bias.size())

        output = F.linear(
            middle, weight=adapter_weights.up.weight, bias=adapter_weights.up.bias
        )
        
        # print("Shapes after the second F.linear:")
...
class AdapterLayersOneHyperNetController(nn.Module):
    ...
    def __init__(self, config, num_layers=6):
        ...
        # print("embedd_dim", config.task_embedding_dim)
...
class TaskHyperNet(nn.Module):
    ...
    def forward(self, task_embedding):
        ...
        embedding = self.task_embeding_generator(task_embedding)
