...
class DataTrainingArguments:
    ...
    speaker_embedding_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "File path to embedding.csv file, containing vector representing speakers."
        },
    )
    metadata_transposer: Optional[dict[str, dict[str, str]]] = field(
        default="",
        metadata={"help": "Maps the categories of metadata to bool flags"},
    )
    columns_from_datasamples: Optional[dict[str, List[str]]] = field(
        default="",
        metadata={"help": "Adds more metadata contained only in the labels"},
    )
