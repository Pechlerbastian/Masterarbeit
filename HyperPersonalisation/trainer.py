...
import sys
# sys.path.insert(0, '/data/eihw-gpu1/pechleba/HyperPersonalisation')
import torch
import pyreadstat
import datasets
import ast
...
from third_party.trainers import Wav2Vec2Trainer
...
def main(fold=0):
    ...
    metadata = pd.read_csv(data_args.metadata_file, delimiter=',')

 
    # start changed Masterarbeit Bastian Pechler
    
    if data_args.metadata_transposer:
        transposed = []
        # replacement of columns, if metadata_transposer is not empty
        # e.g. Geschlecht with state (1, 2, 3) -> one-choice booleans: male, female, divers
        for column in data_args.metadata_columns:
            if column in data_args.metadata_transposer:
                transposer_dict = data_args.metadata_transposer[column]
                metadata = metadata.assign(
                    **{new_value: (metadata[column] == int(new_key)).astype(int) for new_key, new_value in
                    transposer_dict.items()})
                transposed.append(column)

        metadata = metadata.fillna(metadata.mean())

        # keep track of additional columns
        additional_columns = []

        for transposed_col in transposed:
            additional_columns.extend(metadata[data_args.metadata_transposer[transposed_col].values()]
                                    .select_dtypes(["number"])
                                    .columns)
        logger.info(f"Dropping non-numeric columns from metadata.")
        # concatenate metadata of type numbers with according subject IDs and the columns inserted by replacing
        # original columns to achieve one-hot encodings instead categories

        metadata = pd.concat(
            (
                metadata[data_args.subject_column],
                metadata[data_args.metadata_columns].select_dtypes(["number"]),
                metadata[additional_columns]
            ),
            axis=1,
        )

        # drop replaced categorical columns
        metadata.drop(columns=transposed, inplace=True)
    else:
    ...
    if data_args.speaker_embedding_file:
        # metadata_help = metadata[data_args.metadata_columns]
        data_args.metadata_columns.append("embedding")
        embedding = pd.read_csv(data_args.speaker_embedding_file, delimiter=',')
        embedding['embedding'] = embedding['embedding'].apply(lambda x: ast.literal_eval(x))

        expanded_embedding = pd.DataFrame(embedding['embedding'].tolist(), columns=[f'embedding_{i}' for i in range(
            len(embedding['embedding'].iloc[0]))])

        # Concatenate expanded_embedding DataFrame with original embedding DataFrame for keeping subject
        embedding = pd.concat([embedding, expanded_embedding], axis=1)

        # Drop original 'embedding' column
        embedding = embedding.drop(columns=['embedding'])
        
        metadata = pd.merge(metadata, embedding, on="subject")
    ...
    # start changed for Masterarbeit Bastian Pechler
    if data_args.columns_from_datasamples:
        for key, value_list in data_args.columns_from_datasamples.items():
            if key in data_args.metadata_columns:
                data_args.metadata_columns.remove(key)

            data_args.metadata_columns.extend(value_list)
            one_hot_encoded = pd.get_dummies(df[key])
    ...
    if training_args.train_adapters:
        ...
        print("length", len(data_args.metadata_columns))
